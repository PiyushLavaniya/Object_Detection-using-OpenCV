...
def process_video(video_path):
    ...
    while True:
        ...
        if count_edges > 4:
            cv2.putText(frame_edges, "White strip exists - Edges", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        else:
            cv2.putText(frame_edges, "White strip does not exist", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        if count_binary > 4:
            cv2.putText(frame_binary, "White strip exists - Binary", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        else:
            cv2.putText(frame_binary, "White strip does not exist", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
...
def main():
    ...
    if options == 'Image':
        ...
        if uploaded_file is not None:
            ...
            if num_contours > 6:
                st.write("White Strip detected, passed.")
            else:
                st.write("White Strip could not be detected, Failed.")
